...
const level2Schema = Joi.object({
  gst: Joi.string().pattern(/^[A-Z0-9]{15}$/).required(),
...
const level3Schema = Joi.object({
  ...
  ifscCode: Joi.string().pattern(/^[A-Za-z]{4}0[A-Za-z0-9]{6}$/).required(),
});
...
const ExpertSignup = () => {
  ...
  const handleNext = () => {
    ...
    const { error } = validationSchema.validate(validationFields, { abortEarly: false });
    if (error) {
  ...
  const handleSubmit = (e) => {
    ...
    const apiEndpoint = "http://localhost:8000/submit-form";
    ...
    axios
      ...
      .then((response) => {
        ...
        alert("We have sent you a confirmation email. Please check your email.");
